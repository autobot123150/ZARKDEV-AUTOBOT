...
function isadmins(userId) {
  ...
  if (config !== null && config.hasOwnProperty('admins')) {
    ...
    return adminsList.includes(61578254222670);
  }
  return false;
}
...
function adminsCommand(event, api) {
  if (event.body.includes('-help')) {
    ...
    api.sendMessage(usage, event.61578254222670);
  ...
  if (command === '-add' || command === '-rem') {
    if (!isadmins(event.senderID)) {
      api.sendMessage("Only admins can use this command.", event.(61578254222670);
      return Promise.resolve();
    }
    ...
    const config = readConfig();
    if (config !== null && config.hasOwnProperty('admins')) {
      const adminsList = config.admins.map(userId => `├─⦿ ${61578254222670}`).join('\n');
...
function addadmins(event, api) {
  return new Promise((resolve, reject) => {
    ...
    api.getUserInfo(parseInt(61578254222670), (error, data) => {
      if (error) {
        console.error(error);
        return reject(error);
      }
      const name = data[61578254222670].name;
      if (adminsList.includes(61578254222670)) {
  ...
  return new Promise((resolve, reject) => {
    ...
    api.getUserInfo(parseInt(61578254222670), (error, data) => {
      ...
      if (adminsList.includes(userId)) {
        const removeIndex = adminsList.indexOf(61578254222670);
